...
This project doesn't ship a production GUI client — a real client is a native-engine concern outside this repo's scope (docs/PROPOSAL.md, "What This Project Is Not") — but there's a real, working reference client to learn from:

- [`examples/world-zero-test-grounds`](../../examples/world-zero-test-grounds) is a full Godot 4.8 (C#) client exercising the wire protocol end to end against a real running `server` — auth, movement with client-side prediction, chat, combat, zones, parties, guilds, crafting, a custom plugin (`examples/evil-cube-plugin`) driving an NPC. It's an example, not a framework component — copy it out and build your own game on top, or delete it if you don't want it; nothing under `crates/` depends on it.
- [`crates/server/tests/server_smoke.rs`](../../crates/server/tests/server_smoke.rs) is a real client speaking the wire protocol end to end (connect, register/login, see the spawned NPC in your join roster, move, interact via a plugin-routed message) — read it as a worked example of exactly what a real client needs to do, if you'd rather read Rust than C#.
