...
**The UDP/DTLS channel — opt-in, not required.** [`docs/specs/Networking_Spec.md`](../specs/Networking_Spec.md#dtls-udp-channel) specs an unreliable UDP channel (DTLS-secured, reusing the same cert/key as TCP) for high-frequency, loss-tolerant traffic, and as of [#295](https://github.com/LunarVagabond/WorldZero/issues/295) it's real end to end — `server` stands up a UDP listener at `WZ_GATEWAY_UDP_ADDR` (default `127.0.0.1:7901`) alongside the TCP one. TCP-only remains a fully supported, unaffected way to build a client: skip this and every message, `Move` included, stays on the single TCP+TLS connection exactly as before #295 — the association below is never assumed. If you do want the lower-latency path for movement, see §4's "Moving movement onto UDP" below for the handshake.
...
**Moving movement onto UDP (optional, #295).** Once you have a `session_token` (from `Authenticated`, §3a) and have joined (`Joined`, §3d), you can open a second, independent DTLS connection to `WZ_GATEWAY_UDP_ADDR` and send `BindUdp { session_token }` as your first datagram — the same envelope shape and `message_type` 200 as everything else in this section, just carried over UDP instead of TCP. The server replies `UdpBound {}` over that same UDP path once it resolves the token back to your connection. From then on, send `Move`/`Ping` over the UDP connection instead of TCP, and every `message_type` 200 reply addressed to you — your own `Moved`/`Rejected`/`Pong`, and any `Moved`/`EntitySpawned`/`EntityDespawned` broadcast about another entity you observe — arrives over UDP too. Everything outside `message_type` 200 (auth, realm/character selection, chat, party/guild/trade) is unaffected either way and always stays on TCP. The association only lasts for this TCP connection's lifetime; there's no reconnect/re-bind story yet if your UDP source address changes mid-session (e.g. a NAT rebind). The Godot reference client's [`Net/DtlsConnection.cs`](../../examples/world-zero-test-grounds/Net/DtlsConnection.cs) is a real worked example of this handshake.
