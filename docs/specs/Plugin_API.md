...
| `move-entity` | `func(entity-id: string, x: f64, y: f64, z: f64) -> result<_, string>` | Queues a move applied and validated on the zone's next tick through the same path a player's own movement goes through (`world::Zone::request_move`, 3D-aware since #249) — never a direct position write. **Not an instant-reposition/teleport primitive**: the target is bounded by the exact same per-tick speed-cap/collision validation a player's own `Move` goes through, so a target far enough away to exceed the speed cap is clamped or rejected the same way an out-of-range player move would be. There is no privileged host function that sets a position directly, admin teleport included. |
...
| `plugin.host_api_version` | string | Must equal `plugin_host::HOST_API_VERSION` (currently `"0.13.0"`, matching the WIT package version above) or the plugin is refused before instantiation. |
