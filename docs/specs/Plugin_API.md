...
| `move-entity` | `func(entity-id: string, x: f64, y: f64, z: f64) -> result<_, string>` | Queues a move applied and validated on the zone's next tick through the same path a player's own movement goes through (`world::Zone::request_move`, 3D-aware since #249) — never a direct position write. |
