...
## Wire schema (decision: #109)

The `Payload encoding` column above is JSON today but is being migrated to **protobuf** (Rust side via `prost`) as the canonical wire schema for every `message_type`, decided in #109. Protobuf was chosen over staying with JSON or moving to FlatBuffers/Cap'n Proto for its codegen coverage across all three target engines (Unity/C#, UE5/C++, Godot via a `protoc` plugin) and because the protocol surface is still small enough to migrate cheaply now rather than later. A `.proto` schema becomes the source of truth this catalog table currently is by hand. This is a deliberately separate concern from the plugin ABI's WASM Component Model/WIT choice — no shared IDL between the two. Migration of `auth`/`chat`/`server::session_protocol` off `serde_json` is tracked as follow-up implementation work, not part of the decision itself.
