...
| 200 | `server` (`server::session_protocol`) | Protobuf (`crates/server/proto/session.proto`) | The phase-1 combined server's movement session (join a zone, move, see other entities move) — gated behind the same auth handshake. |
