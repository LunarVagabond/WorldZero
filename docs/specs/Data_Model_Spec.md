...
- `grants` (optional, #289) — zero or more `{stat, amount}` entries, each a delta applied to a declared stat on a successful craft (e.g. `{stat: profession.blacksmithing_xp, amount: 10}`)

Validated at load (`CraftingSchema::from_yaml`): non-empty recipe list, unique recipe keys, non-empty `inputs` per recipe, positive amounts throughout, (#287) every `item_type` referenced — as an input or the output — exists in the item catalog, with the output additionally required to carry the `craftable_output` tag (see "The item catalog" above), and (#289) every `requires`/`grants` stat key is a real stat declared in `stats.schema.yaml` (`AttributeSchema::declares`, existence-only — same check `equipment.schema.yaml`'s own `stat_deltas` already gets) — an unknown stat key is a load-time error naming the recipe, the field (`requires`/`grants`), and the bad key — same fail-loud-at-startup discipline every other declared schema in this codebase uses.
...
Once the exchange commits, every declared `grants` delta (#289) is applied via `CharacterStore::apply_stat_delta` — the exact same write path `equipment.rs`'s `equip_item`/`unequip_item` already use for `stat_deltas`, so `stats.schema.yaml`'s declared `min`/`max` bounds are enforced for real, not bypassed. This happens *after*, not inside, the craft's own transaction — same "best-effort, not fully transactional" stance `equipment.rs`'s own doc comment already takes for its stat-delta application — so a `grants` delta rejected by its stat's declared bounds doesn't undo an already-committed craft.

`requires`/`grants` are the only stat-facing primitives core adds to crafting. A "profession" is nothing more than a dev-declared stat (e.g. `profession.blacksmithing_xp`) that a recipe's `grants` writes to and another recipe's `requires` reads back — core has no concept of a profession, level, or XP curve of its own; that's entirely the dev's own data/config built on these two primitives. Quality tiers and success chance remain the `on-craft-complete` plugin hook's job (`docs/specs/Plugin_API.md`).
