...
- `requires` (optional, #289) — zero or more `{stat, min}` entries gating the recipe behind a minimum *current* value for a declared stat (e.g. `{stat: profession.blacksmithing_level, min: 5}`)
- `grants` (optional, #289) — zero or more `{stat, amount, below?, chance?}` entries, each a delta applied to a declared stat on a successful craft (e.g. `{stat: profession.blacksmithing_xp, amount: 10}`). `below` (optional) skips the grant entirely — no delta at all, not even a clamped one — once the character's *current* value for `stat` is no longer strictly less than it, letting a dev cut off a recipe's XP well before the stat's own declared max (e.g. "outleveled this recipe, still craftable, no longer teaches you anything"). `chance` (optional, `(0.0, 1.0]`, defaults to always-applies when omitted) is the independent-per-grant probability the delta fires at all on an otherwise-successful craft.

Validated at load (`CraftingSchema::from_yaml`): non-empty recipe list, unique recipe keys, non-empty `inputs` per recipe, positive amounts throughout, (#287) every `item_type` referenced — as an input or the output — exists in the item catalog, with the output additionally required to carry the `craftable_output` tag (see "The item catalog" above), (#289) every `requires`/`grants` stat key is a real stat declared in `stats.schema.yaml` (`AttributeSchema::declares`, existence-only — same check `equipment.schema.yaml`'s own `stat_deltas` already gets), and a declared `chance` is within `(0.0, 1.0]` — an unknown stat key or an out-of-range `chance` is a load-time error naming the recipe, the field, and the bad value — same fail-loud-at-startup discipline every other declared schema in this codebase uses.

**The atomic exchange.** `character::CharacterStore::craft_item` is the one write path: given a character id and a resolved `Recipe`, it first rejects the attempt outright (nothing touched) if any declared `requires` isn't met — the crafting character's *current* value for `requires[].stat` (`AttributeSchema`'s declared default if never written) is below `requires[].min`. If every `requires` is met, it runs one Postgres transaction that locks every declared input row (`FOR UPDATE`), verifies each is present in at least the declared amount, consumes all inputs and grants the output, then applies every declared `grants` delta (skipping any whose `below` ceiling has already been reached or whose `chance` roll misses) — all in that same transaction — or, if any input is insufficient, rolls back and changes nothing at all. This is deliberately not built on `inventory.rs`'s `grant_item`/`remove_item` (each of those is its own non-transactional statement, by design — see that module's doc comment); a craft's multi-row exchange needs the all-or-nothing guarantee those two don't individually provide, same reasoning `transfer::TransferExecutor::transfer_inner` already applies to a realm move. The output's capacity check reuses `grant_item`'s own "a *new* stack past `InventoryConfig::max_distinct_item_types` is rejected" rule.

A `grants` delta is applied via `CharacterStore::apply_stat_delta_clamped_tx` — *clamped* to the stat's declared `min`/`max` (`AttributeSchema::clamp`), not rejected the way `equipment.rs`'s `equip_item`/`unequip_item` reject an out-of-bounds `stat_deltas` write. This is deliberate and is what makes running the grant inside the craft's own transaction safe: a capped growth stat like profession XP already at its max must never fail (or roll back) the craft that produced it — it should just stop growing. Rejecting instead of clamping here would mean a maxed-out profession silently blocks crafting the recipe forever.

`requires`/`grants` are the only stat-facing primitives core adds to crafting. A "profession" is nothing more than a dev-declared stat (e.g. `profession.blacksmithing_xp`) that a recipe's `grants` writes to and another recipe's `requires` reads back — core has no concept of a profession, level, or XP curve of its own; that's entirely the dev's own data/config built on these two primitives (`below` and `chance` included — they're generic per-grant modifiers, not profession-specific). Whether the craft *itself* succeeds or fails, and any quality-tier rolls on the output, remain the `on-craft-complete` plugin hook's job (`docs/specs/Plugin_API.md`) — `grants[].chance` only governs whether one already-successful craft's stat delta fires.
