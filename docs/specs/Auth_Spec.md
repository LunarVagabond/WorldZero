...
- Storage sits behind an `AccountStore` trait (`create`/`find_by_username`/`find_by_id`) so the provider itself doesn't care where accounts live. `PostgresAccountStore` is the real implementation, backed by the `accounts` table (`db/migrations/0001_create_accounts/`) — `id UUID PRIMARY KEY`, `username TEXT UNIQUE`, `password_hash TEXT`, `created_at TIMESTAMPTZ`. `InMemoryAccountStore` exists only for tests. `find_by_id` (#195) exists specifically for session resumption: a `Resume{ session_token }` only resolves an `AccountId` via `SessionManager::resolve`, never a username, but `Authenticated`'s wire shape still needs one to reply with.
...
- A connection's very first envelope must be `message_type` 1: `Register { username, password }`, `Login { username, password }`, or `Resume { session_token }` — `UsernamePasswordProvider::register`/`verify_credentials` run against the first two exactly as described above (Argon2id hashing, generic "invalid credentials" for both wrong-password and nonexistent-username); `Resume` (#195) skips credential verification entirely and instead resolves the presented `session_token` via `SessionManager::resolve`.
- On success, the server replies `Authenticated { account_id, username, session_token, roles }` — for `Register`/`Login` this is a freshly `issue_session`-minted token; for `Resume` it's the same token the client presented, its TTL renewed in place. Either way the `account_id` from here on is what every other `message_type` on the connection trusts as that connection's identity — nothing downstream (chat's `Join`/`Leave`/`Send`) accepts a client-claimed identity anymore. `roles` (#248) is `AccountRoleStore::roles_for(account_id)` at the moment of authentication — empty for an account with none — so a client can build role-gated UI (an admin panel, say) directly instead of inventing an ad-hoc convention. `chat::bin::gateway_server`'s standalone demo has no `AccountRoleStore` wired in and always sends `roles: []`; `server`'s combined process is the real implementation.
- On failure, the server replies `Error { message }` and closes the connection — no retry-on-the-same-connection; the client reconnects to try again. For `Resume` specifically, an unknown or expired token is exactly this case: a clear `Error`, with the client expected to fall back to a real `Login` — never a silent or ambiguous failure.
- **Session token lifecycle (#195, a deliberate choice, documented here rather than left implicit):** bearer-token model — presenting the raw `session_token` is sufficient to resume, no second factor, matching how the token was already stored/used before resumption existed. **Sliding expiration, same token reused on every resume** — `SessionManager::resolve` renews the Redis key's TTL back to the full 24h window on every successful resolve rather than rotating to a new token, the same shape an ordinary web session cookie already has. This keeps the client-side contract simple (whatever token `Authenticated` last handed back keeps working for as long as reconnects happen within the TTL window, nothing extra to track) at the cost of the token being reusable indefinitely as long as it's presented at least once per window — an accepted v0 tradeoff, not a byproduct of not having considered token rotation.
...
**CLI:** `make role ARGS="grant <username> <role>"` (also `revoke`/`list`) — `crates/auth/src/bin/role.rs`, `cargo run -p auth --bin role -- <command> [args...]` directly. Mirrors `realm-directory`'s `realm` CLI: resolves a username to an `AccountId` via `AccountStore::find_by_username`, then calls the matching `AccountRoleStore` method. Needs `WZ_POSTGRES_*`.
...
**Client-facing roles (#248):** the same `roles_for(account_id)` call is made a second time (`server::session::handle_session`, before `Authenticated` is sent — plugin-facing `caller_role` still resolves separately at world-join, since no `EntityId` exists yet at auth time) so a *connecting client*, not just a plugin, can see its own roles — see "Gateway handshake" above. Global-scope v0 only, same as `caller_role`.
