...
**What running a content pack actually does (#45):** `server::main` (the combined-process binary) checks for `content-pack.yaml` in the config directory before anything else — present, it spawns one zone-service instance (`world::Zone` + its own `world_actor::spawn_world_actor` task and `Sessions` map) **per zone in the pack**, all inside the same process. Absent, it falls back to a single zone loaded from `zone.manifest.yaml` (the original Phase 1 behavior — every `zone.manifest.yaml`-only deployment keeps working unmodified). A player walking through a `links[]` edge transitions live between zone-service instances — no client reconnect, no new gateway handshake, the same TCP connection and `entity_id` carry straight through (`server::session_protocol::ServerMessage::ZoneChanged`). See `crates/server/src/zone_registry.rs`'s module doc comment for exactly what is and isn't built yet (the *set* of zones is fixed at process startup; independently starting/stopping one zone-service instance without restarting the process is deferred to `realm-directory`, #47). The shipped example (`config/content-pack.example.yaml`, two zones: `greenwood-forest` and `stonebridge-village`) demonstrates this end to end — see `crates/content/src/content_pack.rs`'s `the_shipped_example_content_pack_loads` test.
