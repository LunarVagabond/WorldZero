...
**Optional services can be turned off without a fork.** Not every crate is required for every deployment — `chat` is the first example: an operator who has no use for it should be able to leave it out of their combined-process `server` while keeping everything else. This is a runtime config toggle (e.g. `services.chat.enabled`), not a Cargo compile-time feature — `server` always links every crate it supports, and config decides at startup which optional services actually stand up their routes/tasks/DB pool. This only applies to crates explicitly documented as optional (`chat`, `transfer`, `plugin-host`, `realm-directory` per the roadmap); the Phase 1 core (`auth`, `character`, `world`, `gateway`, `content`) is not part of this toggle. See decision issue [#91](https://github.com/LunarVagabond/WorldZero/issues/91).
