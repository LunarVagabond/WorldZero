...
- **Navmesh check (#280):** destination must also land inside a real navmesh polygon (`z` included — real interiors/multi-floor support), else `NotWalkable`. This is the actual walkability authority now — a destination inside the flat bounds polygon but off every navmesh polygon (inside a wall, above/below a floor) is still rejected.
